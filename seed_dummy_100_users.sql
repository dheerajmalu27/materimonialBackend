...
--
-- Notes:
-- - This script first removes existing records from core app tables,
--   resets identities, and then inserts a fresh batch.
...
-- Delete existing records and reset PK/identity sequences
TRUNCATE TABLE
  messages,
  conversations,
  blocked_users,
...
DO $$
BEGIN
  IF to_regclass('pg_temp._seed_users') IS NOT NULL THEN
    DROP TABLE pg_temp._seed_users;
  END IF;
END
$$;
...
inserted AS (
  ...
  SELECT
    CASE
      WHEN sr.gender = 'male' AND sr.seq = 1 THEN 'seed.male@matrimony.com'
      WHEN sr.gender = 'male' AND sr.seq = 2 THEN 'seed.male.premium@matrimony.com'
      WHEN sr.gender = 'female' AND sr.seq = 1 THEN 'seed.female@matrimony.com'
      WHEN sr.gender = 'female' AND sr.seq = 2 THEN 'seed.female.premium@matrimony.com'
      ELSE lower(format('dummy.%s.%s.%s@matrimony.com', sr.gender, lpad(sr.seq::text, 2, '0'), p.run_id))
    END AS email,
    CASE
      WHEN sr.gender = 'male' AND sr.seq = 1 THEN '9000000001'
      WHEN sr.gender = 'male' AND sr.seq = 2 THEN '9000000003'
      WHEN sr.gender = 'female' AND sr.seq = 1 THEN '9000000002'
      WHEN sr.gender = 'female' AND sr.seq = 2 THEN '9000000004'
      WHEN sr.gender = 'male' THEN '8' || substr(p.run_id, 1, 3) || lpad(sr.seq::text, 6, '0')
      ELSE '7' || substr(p.run_id, 1, 3) || lpad(sr.seq::text, 6, '0')
    END AS mobile,
    '$2b$10$o2sVN1OTStdQ7ss8yreJS.05u7CZzXsLSErfOOSLClDJm7p2d9p4a' AS password_hash,
    sr.gender AS gender,
    TRUE AS is_active,
...
-- premium activation for 2 fixed accounts (1 male + 1 female)
INSERT INTO user_activities (user_id, action, description, ip_address, user_agent)
SELECT
  su.id,
  ...
  json_build_object(
    ...
    'planName', 'Premium',
    'startedAt', to_char((now() - interval '1 day') AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'),
    'expiresAt', to_char((now() + interval '365 days') AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"')
  )::text,
  '127.0.0.1',
  'seed-script/1.0'
FROM _seed_users su
WHERE su.email IN ('seed.male.premium@matrimony.com', 'seed.female.premium@matrimony.com');
